            ...
            <div align = "center">
                <form method="POST" enctype="multipart/form-data">
                    <div align = "center" class="info">
                        ...
                        <div class = "depot-input"><input id = "admission" class = "depot-texte" maxlength = "5" placeholder="Oui/Non" name = "admission" size = "60" type = "text" value = "<?php echo isset($_POST["admission"]) ? $_POST["admission"] : ""; ?>" /></div><br>
    ...
    if (!in_array($ext_synthese, $ext_val) && !in_array($ext_rapport, $ext_val)) {
        ...
        if ($_FILES['synthese']['size'] > $taille_max && $_FILES['rapport']['size'] > $taille_max) {
            ...
            try {
                ...
                echo "<div class = 'erreur'>L'envoi des rapports n'est pas bien déroulé. Veuillez contacter le responsable pour avoir plus d'information</div>";
            }

            // Ajouter les infos dans la table "stages"
            $requete_ajout_stage = $pdo->prepare('INSERT INTO stages VALUES("",?,?,?,?,?,?,?,?,?,?,?,?)');
            $requete_ajout_stage->execute([$id_utilisateur, $nom, $prenom, $promotion, $entreprise, $ville, $secteur, $sujet, $admission, $rapport_stage, $note_synthese, $resume_stage]);
